...
import {
  ...
  ComplianceKpiCards,
  ComplianceMultiProcedureTrendChart,
...
const PROCEDURE_OPTIONS = [
  { code: '', name: 'Tất cả 3 quy trình' },
  { code: 'IV_INJECTION', name: 'Tiêm tĩnh mạch' },
  { code: 'IV_INFUSION', name: 'Truyền tĩnh mạch' },
  { code: 'IV_CATHETER', name: 'Tiêm qua kim lưu' },
];

export function QtktTechnicalComplianceReportPanel() {
  const { user } = useAuth();
  ...
  const query = useMemo((): reportSvc.ComplianceQuery => ({
    ...
    page,
    size,
  }), [yearMonth, departmentId, procedureCode, search, resultFilter, sortDir, page, size]);
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      setReport(await reportSvc.fetchTechnicalComplianceReport(query));
    } catch {
      setError('Không tải được báo cáo tuân thủ quy trình kỹ thuật.');
    } finally {
      setLoading(false);
    }
  }, [query]);
  ...
  async function exportFile(kind: 'excel' | 'pdf') {
    setExporting(kind);
    try {
      if (kind === 'excel') await reportSvc.downloadTechnicalComplianceExcel(query);
      else await reportSvc.downloadTechnicalCompliancePdf(query);
    } catch {
  ...
  const trendByProcedure = report?.trendByProcedure ?? {};

  return (
    <Box>
      <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={1} sx={{ mb: 1.5 }}>
        <Box>
          <Typography variant="subtitle2" fontWeight={800}>Tuân thủ quy trình kỹ thuật</Typography>
          <Typography variant="caption" color="text.secondary">3 quy trình tiêm/truyền TM · Đạt ≥ 7/10</Typography>
        </Box>
      ...
      <Paper elevation={0} sx={{ p: 1.5, mb: 2, borderRadius: 2.5, border: `1px solid ${alpha('#64748b', 0.12)}` }}>
        <Stack direction="row" spacing={1.15} alignItems="center" sx={{ flexWrap: { xs: 'wrap', md: 'nowrap' } }}>
          ...
          <TextField select size="small" label="Quy trình" value={procedureCode} sx={{ minWidth: 170, flex: { md: '0 0 190px' } }} onChange={(e) => { setProcedureCode(e.target.value); setPage(0); }}>
            {PROCEDURE_OPTIONS.map((p) => <MenuItem key={p.code || 'all'} value={p.code}>{p.name}</MenuItem>)}
          </TextField>
          <TextField
            size="small"
            label="Tìm theo tên"
      ...
      {report && (
        <Stack spacing={2}>
          ...
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1fr 1fr' }, gap: 2 }}>
            <ReportChartCard title="Xu hướng tuân thủ QTKT" subtitle="Tỷ lệ đạt theo thời gian trong kỳ">
              <ComplianceTrendChart trend={report.trend} />
            </ReportChartCard>
            <ReportChartCard title="So sánh theo quy trình" subtitle="Tiêm · Truyền · Kim lưu">
              <ComplianceMultiProcedureTrendChart trendByProcedure={trendByProcedure} />
            </ReportChartCard>
          </Box>
          <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2.5, border: `1px solid ${alpha('#64748b', 0.12)}` }}>
            <ComplianceBucketTable title="Thống kê theo quy trình" rows={report.byProcedure ?? []} nameHeader="Quy trình" nameKey="procedureName" />
          </Paper>
          <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2.5, border: `1px solid ${alpha('#64748b', 0.12)}` }}>
            <Typography variant="subtitle2" fontWeight={800} sx={{ mb: 1 }}>Thống kê theo khoa</Typography>
            <ComplianceDepartmentTable rows={report.byDepartment} showProcedureBreakdown onSelectDepartment={(id) => { setDepartmentId(id); setPage(0); }} />
          </Paper>
          <Paper elevation={0} sx={{ p: 1.5, borderRadius: 2.5, border: `1px solid ${alpha('#64748b', 0.12)}` }}>
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ mb: 1.25 }} alignItems={{ sm: 'center' }}>
              <Typography variant="subtitle2" fontWeight={800} sx={{ flex: 1 }}>Danh sách đánh giá chi tiết</Typography>
