...
export function QuangTrungSupplementForm({
  open,
  employeeId,
  employeeName,
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      att.fetchQuangTrungSupplement(employeeId, initialWorkDate).catch(() => ({ exists: false as const })),
    ])
      .then(([sch, info]) => {
        if (cancelled) return;
        setSchedule(sch);
  ...
  async function handleSave() {
    ...
    setErr(null);
    try {
      const payload: att.QuangTrungSupplementBody = {
        workDate,
      ...
      await att.applyQuangTrungSupplement(employeeId, payload);
      onSaved?.();
      onClose();
    } catch {
      setErr('Không lưu được công Quang Trung. Kiểm tra quyền và dữ liệu nhập.');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleDelete() {
    if (!existing) return;
    if (!window.confirm('Xóa công Quang Trung ngày này? Giờ chấm công bổ sung sẽ bị gỡ.')) return;
    setLoading(true);
    setErr(null);
    try {
      await att.deleteQuangTrungSupplement(employeeId, workDate);
      onSaved?.();
      onClose();
    } catch {
      setErr('Không xóa được công Quang Trung.');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <Stack spacing={2.5}>
      <InfoBanner>
        {existing ? (
          <>
            Đang <strong>sửa</strong> công Quang Trung của <strong>{employeeName}</strong>. Có thể đổi ca/giờ rồi lưu,
            hoặc xóa công ngày này.
          </>
        ) : (
          <>
            Bổ sung công làm việc tại <strong>Quang Trung</strong> cho <strong>{employeeName}</strong>. Áp dụng trực
            tiếp, <strong>không</strong> trừ phạt quên chấm.
          </>
        )}
      </InfoBanner>
      ...
      {existing && (
        <Chip
          ...
          color="warning"
          variant="outlined"
          label="Đã có công Quang Trung — đang chỉnh sửa"
        ...
        <>
          ...
          <FormSection title="Lý do bổ sung" subtitle="Tuỳ chọn">
            <TextField
              ...
              multiline
              minRows={3}
              placeholder="Ví dụ: Làm việc tại cơ sở Quang Trung, hỗ trợ công tác đặc biệt…"
              value={reason}
      ...
      <Stack direction="row" spacing={1.5} alignItems="center">
        ...
        >
          {loading ? 'Đang lưu…' : existing ? 'Cập nhật công QT' : 'Lưu công Quang Trung'}
        </Button>
      </Stack>
    </Stack>
  );
